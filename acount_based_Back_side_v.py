...
OUTPUT_DIR = os.path.join('back_side_V', 'acoutposition_back_test_down') # Reversed strategy output
INITIAL_ACCOUNT_BALANCE = 2500.0
RISK_PERCENTAGE = 0.03  # 3% of account balance
PROFIT_TARGET_R = 3.0
...
class Trade:
    ...
    def add_chunk(self, add_on_price):
        ...
        new_total_shares = self.position_size + add_on_shares
...
def _execute_trade_logic(stock_df, prev_day_close, loss_threshold, ticker, date_str, risk_per_trade_dollars):
    trades_today = []
...
def run_volatility_backtest(ticker, date_str, loss_threshold, risk_per_trade_dollars, interval):
    ...
    if interval == '5min':
        stock_df = stock_df.resample('5T').agg({
            'open': 'first', 'high': 'max', 'low': 'min', 'close': 'last', 'volume': 'sum'
        }).dropna()
        if stock_df.empty:
            return []
    
    return _execute_trade_logic(stock_df, prev_day_close, loss_threshold, ticker, date_str, risk_per_trade_dollars)

def generate_performance_report(trades_df, loss_threshold, interval, initial_balance):
    if trades_df.empty:
    ...
    # Use the trade-specific risk for R-value calculations
    ...
    # --- Account Equity Curve and Drawdown ---
...
NOTE: A 5% commission/slippage handicap has been applied to all gross profits.
      Risk per trade is {RISK_PERCENTAGE:.2%} of the current account balance.
...
"""
    ...
    report_dir = os.path.join(OUTPUT_DIR, 'reports_txt')
    os.makedirs(report_dir, exist_ok=True)
    report_filename = os.path.join(report_dir, f'performance_report_{loss_pct}pct_loss_{interval}.txt')
...
def run_single_parameter_backtest(args):
    ...
    log_dir = os.path.join(OUTPUT_DIR, 'trade_logs_csv')
    report_dir = os.path.join(OUTPUT_DIR, 'reports_txt')
    os.makedirs(log_dir, exist_ok=True)
    os.makedirs(report_dir, exist_ok=True)
    
    # --- Run for 1-minute interval ---
    account_balance_1min = INITIAL_ACCOUNT_BALANCE
    all_trades_1min = []
    
    desc_1min = f"{loss_pct}% Loss (1min)"
    for _, row in tqdm(candidate_df.iterrows(), total=candidate_df.shape[0], desc=desc_1min, position=position*2, leave=False):
        date_str = row['date'].strftime('%Y-%m-%d')
        risk_per_trade = account_balance_1min * RISK_PERCENTAGE
        results = run_volatility_backtest(row['ticker'], date_str, loss_threshold, risk_per_trade, '1min')
        if results:
            for trade in results:
                # Apply 5% commission handicap to winning trades for account balance calculation
                pnl = trade['pnl_in_dollars']
                if pnl > 0:
                    pnl *= 0.95
                account_balance_1min += pnl
                all_trades_1min.append(trade)
    ...
    if all_trades_1min:
        ...
        filename_1min = os.path.join(log_dir, f'reversal_backtest_results_{loss_pct}pct_1min.csv')
        results_df_1min.to_csv(filename_1min, index=False)
        generate_performance_report(results_df_1min.copy(), loss_threshold, '1min', INITIAL_ACCOUNT_BALANCE)
    
    # --- Run for 5-minute interval ---
    account_balance_5min = INITIAL_ACCOUNT_BALANCE
    all_trades_5min = []
    
    desc_5min = f"{loss_pct}% Loss (5min)"
    for _, row in tqdm(candidate_df.iterrows(), total=candidate_df.shape[0], desc=desc_5min, position=position*2+1, leave=False):
        date_str = row['date'].strftime('%Y-%m-%d')
        risk_per_trade = account_balance_5min * RISK_PERCENTAGE
        results = run_volatility_backtest(row['ticker'], date_str, loss_threshold, risk_per_trade, '5min')
        if results:
            for trade in results:
                pnl = trade['pnl_in_dollars']
                if pnl > 0:
                    pnl *= 0.95
                account_balance_5min += pnl
                all_trades_5min.append(trade)

    if all_trades_5min:
        results_df_5min = pd.DataFrame(all_trades_5min)
        cols = ['date', 'ticker', 'status', 'entry_attempt', 'entry_price', 'stop_loss', 'total_dollar_risk', 'exit_time', 'exit_price', 'pnl_in_dollars', 'max_profit_per_share', 'max_rr_achieved']
        results_df_5min = results_df_5min[cols]
        filename_5min = os.path.join(log_dir, f'reversal_backtest_results_{loss_pct}pct_5min.csv')
        results_df_5min.to_csv(filename_5min, index=False)
        generate_performance_report(results_df_5min.copy(), loss_threshold, '5min', INITIAL_ACCOUNT_BALANCE)
...
if __name__ == '__main__':
    ...
    # --- TEST RUN: Use first 100 candidates for faster testing ---
    # test_candidate_df = unique_candidates_df.head(100)
        
    print(f"--- Preparing to run the REVERSAL backtest on all {len(unique_candidates_df)} candidates. ---")
