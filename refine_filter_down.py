...
OUTPUT_FILE = 'refined_filtered_stocks_down.csv' # Changed output file
DAILY_DATA_DIR = 'daily-historical-DATA'

# --- New Stricter Filter Criteria ---
MIN_VOLUME = 3_000_000
MIN_PRICE = 1.00
# We are looking for a drop of 10% or more, so the change should be -0.10 or less.
MIN_MOVE_DOWN_PCT = -0.10 
...
def refine_stock_list_down():
    """
    Reads the original filtered list and applies stricter criteria
    to find candidates that have experienced a significant price drop.
    """
    ...
    # --- Correctly parse the percentage string ---
    ...
    print(f"Found {len(source_df)} valid candidates after cleaning. Applying filters for downward movement...")
    
    refined_candidates = []
    
    # Use tqdm for a progress bar
    for _, row in tqdm(source_df.iterrows(), total=source_df.shape[0], desc="Refining List (Down)"):
        # This filter specifically looks for stocks that have moved down significantly.
        # Other conditions like 'Gap up' or 'Move up' are ignored.
        if row['condition'] != 'Move down > 15%':
            continue
        ...
        # 3. "Big Move Down" Filter 
        # Convert the positive 'percent_change' from the file to a negative value.
        percent_change_negative = -row['percent_change']
        
        # Keep stocks where the change is -10% or less (e.g., -0.10, -0.15, -0.25)
        if percent_change_negative > MIN_MOVE_DOWN_PCT:
            continue
    ...
    if not refined_candidates:
        print("No candidates met the criteria for a significant downward move.")
        return
