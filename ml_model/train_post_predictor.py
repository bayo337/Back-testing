...
SHAP_SUMMARY_FILE = os.path.join(SCRIPT_DIR, 'post_shap_summary.png')
FEATURE_DIST_DIR = os.path.join(SCRIPT_DIR, 'feature_distributions_post')
TOP_N_FEATURES = 20
...
def generate_model_analysis(model, X_train, y_train, feature_names):
    ...
    """
    print("Generating comprehensive model analysis for Post-Peak model...")
    ...
    plt.title(f'Top {TOP_N_FEATURES} Feature Importances (Post-Peak Model)')
    plt.tight_layout()
    ...
    plt.title(f'SHAP Summary (Top {TOP_N_FEATURES} Features, Post-Peak Model)')
    plt.tight_layout()
...
def main():
    """Main function to train the post-peak model with hyperparameter tuning."""
    ...
    if not post_cols:
        print("Error: No columns with '_post' suffix found.")
        return
    ...
    plt.title('Confusion Matrix (Tuned Post-Peak Model)')
