...
OUTPUT_DIR = os.path.join('GAP_FADE', 'acoutposition back test')
...
class Trade:
    ...
    def __init__(self, ticker, date, entry_price, stop_loss, entry_attempt, full_position_size, total_dollar_risk):
        ...
        self.initial_stop_high = stop_loss # Hypothesis 4: Store initial stop
...
def _execute_trade_logic(stock_df, prev_day_close, gain_threshold, ticker, date_str, risk_per_trade_dollars):
    ...
    entry_attempts = 0
    high_of_day = 0.0 
    high_since_entry = 0.0 # Hypothesis 4: track high since entry
    cumulative_volume = 0
    active_trade = None
    trade_won_today = False # Flag to stop trading after a win
    ...
    # --- ML MODEL ENHANCEMENT: Calculate OBV ---
    ...
    for index, current_candle in analysis_df.iterrows():
        ...
        if state == 'SEARCHING_SETUP' and entry_attempts < 3:
            # Entry logic can now trigger in the pre-market inclusion zone or market hours
            total_gain = (current_candle['close'] - prev_day_close) / prev_day_close
            
            # --- Condition Check: Use cumulative volume from market hours for the volume check ---
            if total_gain >= gain_threshold and cumulative_volume >= 1_000_000 and current_candle['close'] >= 0.50:
                try:
                    ...
                    if prev_candle_index >= 2:
                        ...
                        # Condition: Volatility-based entry trigger
                        ...
                        if current_candle['low'] < trigger_price:
                            ...
                            state = 'IN_TRADE'
                            high_since_entry = current_candle['high'] # Hypothesis 4: Initialize high tracker
                except (KeyError, IndexError):
                    continue # Skip if we can't get previous candles (e.g., at the start of the dataframe)
        
        elif state == 'IN_TRADE':
            high_since_entry = max(high_since_entry, current_candle['high']) # Hypothesis 4: Update high
            ...
            if not active_trade.partially_closed and active_trade.can_add_more and active_trade.chunks_added < 3:
                # --- HYPOTHESIS 4: "NO NEW HIGHS" CONDITION FOR SCALING-IN ---
            ...
            if current_candle['high'] > active_trade.stop_loss:
                # If stop is below initial entry, it's a trailing stop hit for profit
                status = 'STOPPED_OUT'
...
def generate_performance_report(trades_df, gain_threshold, interval, initial_balance):
    ...
    # --- Create a datetime column for proper chronological sorting and plotting ---
    trades_df['date'] = pd.to_datetime(trades_df['date'])
    trades_df['datetime'] = pd.to_datetime(trades_df['date'].dt.date.astype(str) + ' ' + trades_df['exit_time'].astype(str))
    ...
    chart_dir = os.path.join(OUTPUT_DIR, 'equity_charts')
...
NOTE: A 5% commission/slippage handicap has been applied to all gross profits.
      Risk per trade is {RISK_PERCENTAGE:.2%} of the current account balance, capped at $100,000 per trade.
...
def run_single_parameter_backtest(args):
    ...
    for _, row in tqdm(candidate_df.iterrows(), total=candidate_df.shape[0], desc=desc, position=position, leave=False):
        ...
        risk_per_trade = min(account_balance * RISK_PERCENTAGE, 100_000)
        results = run_volatility_backtest(row['ticker'], date_str, gain_threshold, risk_per_trade, interval)
        if results:
    ...
    if all_trades:
        ...
        filename = os.path.join(log_dir, f'volatility_backtest_results_{gain_pct}pct_{interval}.csv')
...
if __name__ == '__main__':
    ...
    # --- FULL RUN: Test all gain thresholds ---
